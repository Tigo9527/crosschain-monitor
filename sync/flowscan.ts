...
async function fetchFlowScanTx(id:string, accessKey:string) {
    let data = {
        "headers": {
            "accept": "*/*",
        ...
        "body": "{\"operationName\":\"TransactionEventsSectionQuery\",\"variables\":{\"id\":\""+id+"\",\"first\":20},\"query\":\"query TransactionEventsSectionQuery($id: ID!, $eventTypeFilter: ID, $first: Int, $after: ID) {\\n  checkTransaction(id: $id) {\\n    transaction {\\n      eventTypes {\\n        id\\n        __typename\\n      }\\n      eventCount\\n      events(first: $first, typeId: $eventTypeFilter, after: $after) {\\n        edges {\\n          node {\\n            index\\n            type {\\n              id\\n              __typename\\n            }\\n            fields\\n            __typename\\n          }\\n          __typename\\n        }\\n        __typename\\n      }\\n      __typename\\n    }\\n    __typename\\n  }\\n}\\n\"}",
        "method": "POST"
    };
    ...
    eArr.forEach(e=>{
        ...
        if (id === 'A.08dd120226ec2213.PegBridge.Burn' && fields[4].value === '1030') {
            let item = {id, to: fields[5].value, value: fields[3].value};
            matchArr.push(item)
    ...
    for (const e1 of arr.filter(e => e.tokenTransfers)) {
        ...
        for (const t of (txArr as any[])) {
            ...
            if (id === 'A.231cc0dbbcffc4b7.ceWBTC' && type === 'Withdraw' && counterpartiesCount === 0) {
                items.push({hash, time, id, value})
                console.log(`flow scan : ${hash} ${time} ${id} x ${value} ${type}`)
                await fetchFlowScanTx(hash, accessKey)
            }
        }
    }
