...
export const TELOS40_BNB = '0x2C78f1b70Ccf63CDEe49F9233e9fAa99D43AA07e' //
...
export const WeMix1111_BTC = '0x2C78f1b70Ccf63CDEe49F9233e9fAa99D43AA07e' //
...
export const ETHEREUM_TOKENS = new Set<string>([
    ...
    BSC_USD, BSC_ETH, BSC_BTC, BSC_USDC, BSC_anyUSD, BSC_anyETH,
    ...
    TELOS40_BTC, optimism10eth, Godwoken71402_USDT, zksync_era324_USDC, POLOYGON_WETH,
    WeMix1111_BTC,
]);
...
FOREIGN_TOKEN_TO_LOCAL.set(ETHEREUM_USDC_TOKEN.toLowerCase(), E_SPACE_USDC.toLowerCase())
...
FOREIGN_TOKEN_TO_LOCAL.set(BSC_anyETH.toLowerCase(), E_SPACE_ETH.toLowerCase())
FOREIGN_TOKEN_TO_LOCAL.set(POLOYGON_WETH.toLowerCase(), E_SPACE_ETH.toLowerCase())
FOREIGN_TOKEN_TO_LOCAL.set(BSC_BTC.toLowerCase(), E_SPACE_WBTC.toLowerCase())
FOREIGN_TOKEN_TO_LOCAL.set(WeMix1111_BTC.toLowerCase(), E_SPACE_WBTC.toLowerCase())
...
FOREIGN_TOKEN_TO_LOCAL.set(TELOS40_BNB.toLowerCase(), E_SPACE_BNB.toLowerCase())
...
export class EventChecker {
    ...
    public platon210425_p: BaseProvider;
    public bitgert32520: BaseProvider;
...
Block Explorer URL: https://stepscan.io/
    ...
    constructor(url: string, tokenAddr:string) {
        ...
        this.platon210425_p = ethers.getDefaultProvider('https://openapi2.platon.network/rpc')
        ...
        for (let log of logs) {
            ...
            for (let eSpaceLog of receipt.logs) {
                ...
                if (// Burn(bytes32 burnId, address token, address account, uint256 amount, address withdrawAccount) // celer
                    ...
                    } else if (fromChainId == 32520) {
                        [provider, mpc, mpcSet] = [this.bitgert32520, '', {}];
                    } else if (fromChainId == 210425) {
                        [provider, mpc, mpcSet] = [this.platon210425_p, '', {}];
